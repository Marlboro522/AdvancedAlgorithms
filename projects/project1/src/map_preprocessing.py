...
#     G, filename="preprocessing_output/transit_nodes_centrality.parquet"
# ):
#     """
#     Computes betweenness centrality for the entire graph,
#     but saves only the centrality values for transit nodes.
...
#     Returns:
#         dict: Dictionary of betweenness centrality values for transit nodes.
#     """
...
#     # Step 1: Load transit node candidates (nodes that cross grid boundaries)
#     transit_nodes = load_boundary_crossing_nodes()

#     print(
#         "[INFO] Computing betweenness centrality for the full graph... This may take time."
#     )

#     # Step 2: Compute betweenness centrality for all nodes
#     centrality = nx.betweenness_centrality(G, weight="length")

#     # Step 3: Keep only values for transit nodes
#     filtered_centrality = {
#         node: centrality[node] for node in transit_nodes if node in centrality
#     }

#     # Step 4: Convert to DataFrame and save to Parquet
#     df = pd.DataFrame(
...
#         percentile (int): Percentile threshold (default = 95).
...
def precompute_nearest_transit_nodes(
    ...
    # Initialize a shared counter **inside the worker pool**, not in `starmap()`
